...
// === SWIPER INSTANCES ===
let newSwiper, starSwiper;

// === INICIALIZAR SWIPER ===
function initSwipers() {
  if (newSwiper) newSwiper.destroy();
  if (starSwiper) starSwiper.destroy();
  ...
  starSwiper = new Swiper('.star-swiper', {
    slidesPerView: 1,
    spaceBetween: 10,
    loop: true,
...
// === RENDER CARRUSELES ===
function renderCarousel(trackId, items, isNewLaunch) {
  const track = document.getElementById(trackId);
  if (!items.length) {
    track.innerHTML = '<div class="swiper-slide"><p style="text-align:center;color:#999;padding:40px;">No hay productos</p></div>';
    return;
  }

  track.innerHTML = items.map(p => `
    <div class="swiper-slide" onclick="window.location.href='product.html?id=${p.id}'">
  ...
  if (items.length > 1) {
    track.innerHTML += items.slice(0, 2).map(p => `
      <div class="swiper-slide" onclick="window.location.href='product.html?id=${p.id}'">
        <img src="${p.imagen}" alt="${p.nombre}" loading="lazy">
      </div>
    `).join('');
...
// === BÚSQUEDA, UI, TOAST, etc. (igual que antes) ===
let searchTimeout;
...
onSnapshot(collection(db, "productos"), (snapshot) => {
  ...
  renderCarousel('newProductsTrack', allProducts.filter(p => p.type?.carrusel), true);
  renderCarousel('starTrack', allProducts.filter(p => p.type?.anuncio), false);
  renderGrid();
});
